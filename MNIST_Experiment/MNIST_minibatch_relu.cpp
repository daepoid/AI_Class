...
//double Delta3[SZ3];
//
//double relu_deriv1[SZ1];
//double relu_deriv2[SZ2];
//
...
//void accumulate_gradients_of_parameters();
//void Do_train();
//
//double Do_test();
//double getRandNum();
//double Compute_relu_F1_from_F0(int i);
//double Compute_relu_F2_from_F1(int i);
...
//  for (int row = 0; row < ROW; row++) {
//    for (int column = 0; column < COLUMN; column++) {
//      F0[row * ROW + column] = Train_Data[index][row][column] / 255.0;
//
//    }
//  }
//
//  for (int i = 0; i < SZ1; i++) {
//    F1[i] = Compute_relu_F1_from_F0(i);
//    relu_deriv1[i] = F1[i] > 0 ? 1 : 0;
//  }
//
//  for (int i = 0; i < SZ2; i++) {
//    F2[i] = Compute_relu_F2_from_F1(i);
//    relu_deriv2[i] = F2[i] > 0 ? 1 : 0;
//  }
//
//  for (int i = 0; i < SZ3; i++) {
...
//    }
//    for (int j = 0; j < SZ1; j++) {
//      W1_gradient[i][j] += F0[i] * Delta1[j];
//      if (i < SZ1 && j < SZ2) {
//        W2_gradient[i][j] += F1[i] * Delta2[j];
//      }
//      if (i < SZ2 && j < SZ3) {
//        W3_gradient[i][j] += F2[i] * Delta3[j];
//      }
//    }
//  }
//}
...
//        if (index % 2000 == 1) {
//          printf("Train Supervised NN: At epoch %d. learning_rate = %f, minibatch_size = %d, Training example number = %d\n", epoch, learning_rate, minibatch_size, index);
//        }
//        forward_compute(index);
//        backward_compute(index);
//        accumulate_gradients_of_parameters();
//      }
...
//    fprintf(fp_accuracy, "After epoch : %d, learning_rate : %f, minibatch_size = %d, S-NN Accuracy = %f\n", epoch, learning_rate, minibatch_size, Accuracy);
//    fflush(fp_accuracy);
...
//    for (int row = 0; row < ROW; row++) {
//      for (int column = 0; column < COLUMN; column++) {
//        F0[row * ROW + column] = Test_Data[index][row][column] / 255.0;
//      }
//    }
//
//    for (int i = 0; i < SZ1; i++) {
